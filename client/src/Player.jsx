...
export default function Player({ back }) {
  ...
  async function join() {
    await axios.post("http://localhost:5001/api/join", { name, room });
    setJoined(true);
  }
  ...
  async function answer(i) {
    ...
    if (s?.ended) return;
    await axios.post("http://localhost:5001/api/answer", {
      room,
      name,
      answer: i
    });
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      const res = await axios.get("http://localhost:5001/api/status/" + room);
      setS(res.data);
    }, 1000);
